...
매일 한국시간(KST) 새벽 4시부터 9개 금리를 자동으로 수집해서, GitHub Pages 웹사이트에서 확인할 수 있게 해주는 완전 무료 자동화입니다.
...
이후로는 매일 KST 새벽 4시부터 30분 간격으로 7시까지(4:00, 4:30, 5:00, 5:30, 6:00, 6:30, 7:00) 자동으로 실행을 시도합니다 (GitHub Actions의 예약 실행이 가끔 지연/스킵될 수 있어, 늦어도 7시까지는 반영되도록 여러 번 반복합니다 - 먼저 성공하면 이후 시도는 변경 사항이 없어 조용히 끝납니다). 스케줄 시각을 바꾸고 싶으면 `.github/workflows/fetch-rates.yml`의 `cron` 값을 수정하세요. UTC 기준이라 KST보다 9시간 빠릅니다.
