...
매일 한국시간(KST) 새벽 3시대부터 9개 금리를 자동으로 수집해서, GitHub Pages 웹사이트에서 확인할 수 있게 해주는 완전 무료 자동화입니다.
...
이후로는 매일 KST 새벽 3시대부터 30분 간격으로 7시 전까지(3:07, 3:37, 4:07, 4:37, 5:07, 5:37, 6:07, 6:37, 6:52) 자동으로 실행을 시도합니다 (GitHub Actions의 예약 실행은 정각/30분 회차가 특히 많이 스킵되고 20~40분씩 밀리기도 해서, 분을 어긋나게 잡고 일찍부터 여러 번 반복합니다 - 먼저 성공하면 이후 시도는 변경 사항이 없어 조용히 끝납니다). 스케줄 시각을 바꾸고 싶으면 `.github/workflows/fetch-rates.yml`의 `cron` 값을 수정하세요. UTC 기준이라 KST보다 9시간 빠릅니다.
